...
export const userForgotPassword = createAsyncThunk("usersSlice/userForgotPassword",
    async (email: Partial<IUser>) => {
        try {
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(userForgotPassword.fulfilled, (state, action) => {
        state.status = action.payload.status;
        console.log( state.status,action.payload.status );
        
        state.status === 204 ? state.isModalActive = false : state.status = state.status;
      });
  },
});
